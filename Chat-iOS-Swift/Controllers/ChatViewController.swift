...
class ChatViewController: JSQMessagesViewController, UIAlertViewDelegate {
	...
	override func viewDidLoad() {
		...
		if let username = self.user?.username {
			let image = JSQMessagesAvatarFactory.avatarWithUserInitials(avatarLetters(username),
		...
		let systemAvatar = JSQMessagesAvatarFactory.avatarWithImage(UIImage(named: "SHIP"),
	...
	func setupUI() {
		...
		navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Action, target: self, action: #selector(ChatViewController.options))
	}
	...
	func connectWebSocket(host: NSString) {
		SIOSocket.socketWithHost(host as String, response: { (socket: SIOSocket!) -> Void in
			self.socket = socket
			...
			socket.on("userJoined", executeBlock: { (response) -> Void in
				if let json = response as? Dictionary<String, AnyObject> {
					...
					_ = json["numUsers"] as AnyObject? as? NSNumber

					let message = Message(body: NSString(format: "%@ has joined.", username!) as String, sender: "System")
			...
			socket.on("userLeft", executeBlock: { (response) -> Void in
				if let json = response as? Dictionary<String, AnyObject> {
					...
					let message = Message(body: NSString(format: "%@ has left.", username!) as String, sender: "System")
					self.messages.append(message)
	...
	func connectedToChat(numUsers: NSNumber) {
		...
		message = Message(body: NSString(format: "Number of users online: %@", numUsers) as String, sender: "System")
		self.messages.append(message)
	...
	override func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
		...
		if message.sender() == self.senders() {
	...
	override func collectionView(collectionView: JSQMessagesCollectionView!, avatarImageViewForItemAtIndexPath indexPath: NSIndexPath!) -> UIImageView! {
		...
		let avatarImage = JSQMessagesAvatarFactory.avatarWithUserInitials(avatarLetters(message.sender()),
			backgroundColor: UIColor(white: CGFloat(0.85), alpha: 1),
			textColor: UIColor(white: 0.6, alpha: 1),
			font: UIFont.systemFontOfSize(14),
			diameter: UInt(self.collectionView.collectionViewLayout.outgoingAvatarViewSize.width))
	...
	override func collectionView(collectionView: JSQMessagesCollectionView!, layout collectionViewLayout: JSQMessagesCollectionViewFlowLayout!, heightForMessageBubbleTopLabelAtIndexPath indexPath: NSIndexPath!) -> CGFloat {
		...
		if currentMessage.sender() == self.senders() {
			return 0
		}
	...
	override func textViewDidChange(textView: UITextView) {
		...
		self.stopTypingTimer = NSTimer.scheduledTimerWithTimeInterval(1.5, target: self, selector: #selector(ChatViewController.stopTyping), userInfo: nil, repeats: false)
	}
