...
//            setupLogInButton()
	...
	func setupStartButton() {
		if self.user != nil {
			startButton = createButtonWithYCoord(CGRectGetHeight(view.bounds) - 170, title: "enter chat", selector: #selector(IntroViewController.enterButtonPressed(_:)))
		} else {
			startButton = createButtonWithYCoord(CGRectGetHeight(view.bounds) - 170, title: "choose username", selector: #selector(IntroViewController.chooseUsernameButtonPressed(_:)))
		}
	...
	func setupLogOutButton() {
		logOutButton = createButtonWithYCoord(CGRectGetMaxY(startButton!.frame) + 10.0, title: "log out", selector: #selector(IntroViewController.logOutButtonPressed(_:)))
	}

	func setupLogInButton() {
		logInButton = createButtonWithYCoord(CGRectGetMaxY(startButton!.frame) + 10.0, title: "log in", selector: #selector(IntroViewController.logInButtonPressed(_:)))
	}
	...
	func createButtonWithYCoord(yCoord: CGFloat, title: String, selector: Selector) -> UIButton {
		...
		let button = createButtonWithFrame(frame, title: title, selector: selector);
		return button
	}

	func chooseUsernameButtonPressed(sender: UIButton!) {
		let controller = ChooseUsernameViewController(style: UITableViewStyle.Grouped)
		controller.delegate = self
